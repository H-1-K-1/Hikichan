...
$boards = listBoards();
$boards_m = array_chunk($boards, max(1, floor(sizeof($boards)/$options['processes'])));
...
function doboard($board) {
    ...
    // Unified posts table support
    if($options['postmarkup']) {
        $query = query("SELECT `id` FROM ``posts`` WHERE `board` = " . $GLOBALS['pdo']->quote($board['uri'])) or error(db_error());
    ...
    if ($options['threads']) {
        $query = query("SELECT `id` FROM ``posts`` WHERE `board` = " . $GLOBALS['pdo']->quote($board['uri']) . " AND `thread` IS NULL") or error(db_error());
        while($post = $query->fetch()) {
            echo "(/{$board['uri']}/) Rebuilding #{$post['id']}...\n";
            @buildThread($post['id']);
        }
